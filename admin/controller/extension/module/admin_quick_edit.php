...
define('EXTENSION_VERSION',         '5.5.0');
...
class ControllerExtensionModuleAdminQuickEdit extends Controller {
	...
	private $column_defaults = array(
		'aqe_catalog_products'  => array(
			...
			'image'             => array('display' => 1, 'qe_status' => 0, 'editable' => 1, 'index' =>  10, 'align' => 'text-center', 'type' =>   'image_qe', 'sort' => ''                , 'rel' => array(),           'filter' => array('show' => 1, 'type' => 1, 'autocomplete' => false)),
			...
			'requires_shipping' => array('display' => 0, 'qe_status' => 0, 'editable' => 1, 'index' => 210, 'align' => 'text-center', 'type' =>  'yes_no_qe', 'sort' => 'p.shipping'      , 'rel' => array(),           'filter' => array('show' => 1, 'type' => 1, 'autocomplete' => false)),
		...
		'aqe_catalog_categories' => array(
			...
			'image'             => array('display' => 0, 'qe_status' => 0, 'editable' => 1, 'index' =>  10, 'align' => 'text-center', 'type' =>   'image_qe', 'sort' =>                    '', 'filter' => array('show' => 1, 'type' => 1, 'autocomplete' => false)),
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/admin_quick_edit', 'token=' . $this->session->data['token'], true),
			'active'    => true
		);

		$data['save'] = $this->url->link('extension/module/admin_quick_edit', 'token=' . $this->session->data['token'], true);
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], true);
		$data['upgrade'] = $this->url->link('extension/module/admin_quick_edit/upgrade', 'token=' . $this->session->data['token'], true);
		$data['extension_installer'] = $this->url->link('extension/installer', 'token=' . $this->session->data['token'], true);
		$data['services'] = html_entity_decode($this->url->link('extension/module/admin_quick_edit/services', 'token=' . $this->session->data['token'], true));
	...
	public function upgrade() {
		...
		if (!$ajax_request) {
			...
			$this->response->redirect($this->url->link('extension/module/admin_quick_edit', 'token=' . $this->session->data['token'], true));
		} else {
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_enc($response, JSON_UNESCAPED_SLASHES));
			return;
